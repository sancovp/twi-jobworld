...
SKILL (procedure)   ── tells a worker: which CONNECTOR verbs to run, in what order, applying which CLIENT instructions
      │  calls
      ▼
CONNECTOR (code)    ── the ONLY functions: external-effect verbs (pull · video · send · track · host · serve · dashboard · market · suppress · reply)
```
...
| `~/avi-jw/connectors/outreach/` | the outreach connector (`jwout`, 10 verbs) | `connectors/outreach/README.md` |
