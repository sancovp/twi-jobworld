...
<style>
 :root{{--bg:#0d1017;--card:#151a23;--line:#232a37;--dim:#7d8699;--ink:#e7ebf0;
 ...
 *{{box-sizing:border-box}} body{{font:14px/1.55 ui-sans-serif,system-ui,sans-serif;margin:0;background:var(--bg);color:var(--ink)}}
 header{{padding:14px 26px;background:var(--card);border-bottom:1px solid var(--line);display:flex;justify-content:space-between;align-items:baseline}}
 header h1{{margin:0;font-size:15px;font-weight:600}} header .s{{color:var(--dim);font-size:12px}}
 ...
 .fr{{display:grid;grid-template-columns:64px 1fr 86px;align-items:center;gap:10px;margin:3px 0}}
...
<header>
 <h1>Outreach · {_esc(client) or '—'}</h1>
 <span class=s>{total_sends} sends · {booked} booked · {supp} suppressed · auto-refresh 10s</span>
</header>
<main>
