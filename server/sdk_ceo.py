...
try:  # package-relative import (when used as server.sdk_ceo)
...
def _doc_mirror_plugin_root() -> Optional[Path]:
    """Jobworld does NOT auto-load an external plugin. A Jobworld instance's capabilities come from its
    OWN project setting source (its `.claude/{skills,rules,agents}` + `CLAUDE.md`), compiled by the
    render step. So there is no base plugin to discover here — returns None. (Pass an explicit
    `plugins=[...]` to ClaudePMainAgent if a specific instance needs one.)"""
    return None
...
JOBWORLD_CONTEXT_BLOCK = """\
# Jobworld — shared agent context

You are an agent inside a **Jobworld** company instance. THE DIRECTORY CODES THE AGENT: your
identity, skills, and rules come from the `CLAUDE.md` and `.claude/` of the directory you are running
in — and you are ISOLATED from any host/global configuration (you do NOT have the host machine's
persona or skills).

Operate ONLY within your own working directory. The company's departments, agents, CEO prompt, and
MCP wiring are COMPILED from `config.json` by the render step — follow the directed skills
(`skills/run-dept-*`) rather than using raw tools ad-hoc. Treat the Jobworld store (`/api/*`) as
authoritative. Be concrete; produce the actual artifact, not chatter.
"""
...
class ClaudePMainAgent:
    ...
    def __init__(
        ...
        *,
        ...
        # always appended AFTER JOBWORLD_CONTEXT_BLOCK.
...
__all__ = ["ClaudePMainAgent", "ConvoRegistry", "JOBWORLD_CONTEXT_BLOCK"]
